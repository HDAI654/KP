...
def main_page(request):
    if request.user.is_authenticated:
        form = fmessage()
        return render(request, 'index.html', {'form': form, 'user_name': request.user.username})
    else:
...
# this function is for get the every message info from database and send all in formation to front-end
def readmsg(request):
    if request.method == 'POST':
